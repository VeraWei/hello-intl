  ...
  render() {
    ...
    return (
      <div id="ConditionGroup">
        <SubTitle text="过滤条件" btnInvisible={!edittable} btnText="添加/管理" onClick={this.onAddCondition()}/>
        <div className="group">
          {condition && condition.filters.length > 0 ? '' : <div className="empty-title">暂无过滤条件</div>}
          {
            condition && condition.filters.map((item, index) => {
              return (
                <div key={index} style={{fontSize: '12px'}}>
                  <div
                    style={{margin: '8px 0'}}>{index === 0 ? '' : condition.op.toLowerCase() === 'and' ? '且' : '或' }</div>
                  <div>
                    <Button
                      shape="text"
                      style={{color: '#00C1DE', textAlign: 'left', maxWidth: '148px', height: 'unset'}}
                      disabled={!edittable}
